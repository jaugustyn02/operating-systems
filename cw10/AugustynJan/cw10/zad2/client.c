...
int connect_unix_socket(char* path, char* user) {
    struct sockaddr_un addr, bind_addr;
    memset(&addr, 0, sizeof(addr));
    memset(&bind_addr, 0, sizeof(bind_addr));
    addr.sun_family = AF_UNIX;
    bind_addr.sun_family = AF_UNIX;
    snprintf(bind_addr.sun_path, sizeof bind_addr.sun_path,"/tmp/%s%ld", user, time(NULL)%1000);
...
int main(int argc, char** argv) {
    if (strcmp(argv[2], "-w") == 0 && argc == 5){
        ...
        socket_fd = connect_unix_socket(argv[3], argv[1]);
    } else {
